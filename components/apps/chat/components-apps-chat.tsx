...
import TooltipButton from '@/components/components/tooltip/tooltip-button';
import ContactInfoComponent from './UserDetailseBesideChatBox';
    ...
    return (
        <div>
            <div className={`relative flex  h-full gap-5 sm:h-[calc(100vh_-_150px)] sm:min-h-0 ${isShowChatMenu ? 'min-h-[999px]' : ''}`}>
                <div className={`panel   absolute z-10 hidden h-full w-full max-w-xs flex-none space-y-4 overflow-hidden px-4 py-8 xl:relative xl:block ${isShowChatMenu ? '!block' : ''}`}>
                    <div className="flex items-center justify-between">
                        <div className="flex items-center">
                            <select className="w-28 border py-1 px-2 rounded" name="" id="">
                                <option value="" key="">
                                    All
                                </option>
                            </select>
                    ...
                    <div className=" w-full border-b border-white-light dark:border-[#1b2e4b]  dashBoard-bg "></div>

                    <PerfectScrollbar className="chat-users relative h-full  space-y-0.5 ltr:-mr-3.5 ltr:pr-3.5 rtl:-ml-3.5 rtl:pl-3.5 sm:h-[calc(100vh_-_200px)]">
                ...
                <div className="panel flex-1 p-0 whatsApp">
                    {!isShowUserChat && (
                        ...
                        <div className="relative h-full">
                            <div className="flex items-center justify-between p-4">
                                ...
                                <div className="flex gap-3 sm:gap-5">
                                    <select className="border px-5 py-1 rounded bg-sky-100" name="" id="">
                                        <option value=""> Chat Assign </option>
                                    </select>

                                    <button type="button" className="bg-red-100 font-semibold rounded py-1 px-3">
                                        Close
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="absolute bottom-0 left-0 w-full p-4">
                                <div className="relative triangle ">
                                    {attachMent && (
                                        <div className="absolute bottom-1  shadow  rounded p-5 end-8 w-[250px] grid grid-cols-3 gap-4 h-[250px] attachment border border-sky-100 ">
                                            <button className="absolute -end-2 -top-1 bg-gray-700 rounded-full pr-2 pl-2 mr-1 text-white" onClick={openAttachment}>
                                                x
                                            </button>
                                            {attachmentItem.map((item, index) => (
                                                <TooltipButton key={index} icon={item.icon} content={item.name} />
                                            ))}
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="w-full items-center space-x-3 rtl:space-x-reverse sm:flex">
                                    ...
                                    <div className="hidden items-center space-x-3 py-3 rtl:space-x-reverse sm:block sm:py-0">
                                        <button type="button" className=" hover:text-primary  text-[#3E3E3E]" onClick={openAttachment}>
                                            <RiAttachmentLine size={20} />
                                        </button>
                                        <button type="button" className=" hover:text-primary " onClick={() => sendMessage()}>
                                            <IconSend />
                                        </button>
                                    </div>
                                </div>
                ...
                <div
                    className={`panel dashBoard-bg   absolute z-10 hidden h-full w-full max-w-xs flex-none space-y-4 overflow-hidden px-4 py-3 xl:relative xl:block ${isShowChatMenu ? '!block' : ''}`}
                >
                    <PerfectScrollbar className="chat-users relative h-full  space-y-0.5 ltr:-mr-3.5 ltr:pr-3.5 rtl:-ml-3.5 rtl:pl-3.5 sm:h-[calc(100vh_-_200px)]">
                        <ContactInfoComponent userData={userData} />
                    </PerfectScrollbar>
                </div>
            </div>
        </div>
