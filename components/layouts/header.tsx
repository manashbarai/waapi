...
const Header = () => {
    ...
    const [fbLoadingStart, setFbLoadingStart] = useState(false);
    ...
    useEffect(() => {
        ...
        if (!getToken) {
            // navigate('/not-get-token');
        }
        setMyToken(getToken);

        axios
            .post(
                // api + '/vendor/entry/app-data',
                '/api/app/vendor/entry/app-data',
                {
                    companyId: companyId,
            ...
            .then((res) => {
                if (res.data.status == 1) {
                    ...
                    setFacebookAppId(res.data.data.clientId);
                    setFacebookConfigId(res.data.data.configId);
                    ...
                    window.fbAsyncInit = function () {
                        window.FB.init({
                            appId: res.data.data.clientId,
                            cookie: true,
                            xfbml: true,
                            version: res.data.data.appVersion,
                        });
    ...
    const launchWhatsAppSignup = () => {
        ...
        console.log(facebookConfigId);
        console.log(myToken);

        if (facebookSDKLoaded && typeof window.FB !== 'undefined') {
            setFbLoadingStart(true);
            ...
            window.FB.login(
                function (response) {
                    if (response.authResponse) {
                        ...
                        console.log(api + '/vendor/whatsapp/sign-up');
                        console.log({
                            code: code,
                        });
                        console.log({
                        ...
                        // Send the signup code through an API call
                        axios
                            .post(
                                // api + '/vendor/whatsapp/sign-up',
                                '/api/app/vendor/whatsapp/sign-up',
                            ...
                            .then((res) => {
                                setFbLoadingStart(false);
                                console.log('Signup code sent successfully:', res.data);
        ...
        <header className={`z-40 ${themeConfig.semidark && themeConfig.menu === 'horizontal' ? 'dark' : ''}`}>
            <div className="shadow-sm">
                <div className="relative flex w-full items-center bg-white px-5 py-2.5 dark:bg-black">
                    ...
                    {facebookSDKLoaded == true && (
                        <div>
                            <button onClick={launchWhatsAppSignup} id="connect-with-facebook" data-type="add">
                                <i className="bx bxl-facebook-circle"></i> Connect with Facebook
                            </button>
                        </div>
                    )}
                    {/* Todo: code for facebook sign up for whatsapp */}
